const PACKS=[
  {name:"Gói 1",price:290000,coins:1000},
...
$("packages").innerHTML=PACKS.map((p,i)=>`<div class="package" data-i="${i}">🎀<div>${p.name}</div><b>${fmt(p.price)}đ</b><div>🪙 ${p.coins} Coins</div></div>`).join("");
